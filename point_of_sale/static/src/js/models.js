...
odoo.define('point_of_sale.models', function (require) {
    ...
    exports.PosModel = Backbone.Model.extend({
        ...
        after_load_server_data: async function(){
            ...
            // UPDATE 22-02-2022 - Tasa vigente:
            await this.getTasaVigente();
        ...
        // UPDATE 22-02-2022 - Tasa vigente:
        getTasaVigente: async function() {
            const tasas = this.rpc({
                model: 'pos.session',
                method: 'get_currency_rates',
            });
            // let tasaVigente = tasas[1];
            // console.log('Tasa Vigente:');
            // console.log(tasaVigente);
            
            tasas.then((tasaVigente) => {
                console.log(tasaVigente[0]);
                return tasaVigente[0];
             });
            // return tasaVigente;
        },
    ...
    exports.Order = Backbone.Model.extend({
        ...
        // ===================================================================================== //
        getParteEntera: function(numero) {
            numero = Number(numero);
            numero = Math.trunc(numero);
            return numero;
        },
        getParteDecimal: function(numero) {
        ...
        getTramaCantidad: function (cantidad) {
        ...
        getTramaPrecio: function(lenTotalTrama, precio) {
            ...
            let diferenciaLenPrecio = lenTotalTrama - lenPrecioString;
        ...
        /* ---- Inicio de UPDATE 27-02-2022 --- */
        imprimirTxt: function(order, tipoTxt) {
            console.log('Datos de la Factura:');
            console.log(order);
    
            let datosCajero = order.employee;
            let nombreCajero = datosCajero.name;            
            ...
            let nombreCajeroIF = 'i02Usuario: ' + nombreCajero;
            let nombreCajeroUser = 'Usuario: ' + nombreCajero;            
            ...
            for(let i=0; i<detalle_productos.length; i++) {
                ...
                let tramaPrecio = this.getTramaPrecio(16, price)[1];   
            ...
            // ============================= Concatenando datos ============================= //
            let datosIF;
            if( nCantidadNegativa > 0 ) { // <---- Nota de Crédito.
                /*
                #Nota de Credito
            ...
            // UPDATE 28-02-2022:
            // ============================= Métodos de Pago ============================= //
            let metodosDePago = order.paymentlines.models;
            let datosMetodosPago = [];
            for(let i=0; i<metodosDePago.length; i++) {
                let row = metodosDePago[i]; 
                datosMetodosPago[i] = [row['name'], row['amount']];
            }        
            // Imprimiendo datos de los Métodos de Pago:
            for(let i=0; i<datosMetodosPago.length; i++) {
                let row = datosMetodosPago[i]; 
                let name = row[0];
                let amount = row[1];
                amount = amount.toFixed(2);
                let tramaPrecioMetodoPago = this.getTramaPrecio(12, amount)[1];
                let codigoMetodoPago = '';
                switch(name) {
                    case 'Efectivo':
                        codigoMetodoPago = '02';
                        break;
                ...
                let trama = '2' + codigoMetodoPago + tramaPrecioMetodoPago + '\n';
                // datosIF += trama;
                // console.log('Método de Pago: ' + name + ' -  Monto: ' + amount);      
            }        
            datosIF += '101\n';
