...
def _prepare_data(env, data):
    ...
    decimales = 2
    rate_usd = env['res.currency'].search(
        [('name', '=', 'USD')], limit=1).rate
    rate_usd = round(rate_usd, decimales)

    rate_bolivar = env['res.currency'].search(
    ...
    def obtenerPrecioConIva(frase):
        nueva_frase = ''
        if len(frase) > 0:
            for i in range(len(frase)):
                letra = frase[i]
                if letra == ',' or letra.isnumeric():
                    nueva_frase += letra
                # print( letra )     
        res = False
        if nueva_frase != '':
